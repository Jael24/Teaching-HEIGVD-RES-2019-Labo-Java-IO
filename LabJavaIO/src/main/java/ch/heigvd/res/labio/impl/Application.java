...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename) throws IOException {
    ...
    StringBuilder completePath = new StringBuilder(WORKSPACE_DIRECTORY + "/");

    for(String str : tagsList) {
      completePath.append(str).append("/");
    }

    completePath.deleteCharAt(completePath.length()-1);

    new File(completePath.toString()).mkdirs();

    completePath.append("/").append(filename);

    OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(completePath.toString()));
